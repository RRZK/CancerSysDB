        ...
        <s2ui:tab name='userinfo' height='275'>
            <table>
                ...
                <s2ui:textFieldRow name='userDetail.firstName' labelCode='userDetail.firstName.label' bean="${user}"
                                   labelCodeDefault='firstName' value="${user?.details?.firstName}"/>
                <s2ui:textFieldRow name='userDetail.lastName' labelCode='userDetail.lastName.label' bean="${user}"
                                   labelCodeDefault='lastName' value="${user?.details?.lastName}"/>
            ...
            <g:each var="entry" in="${roleMap}">
                <div>
                    <g:checkBox name="${entry.key.authority}"
                                value="${entry.value}"/>${entry.key.authority.encodeAsHTML()}
                </div>
            </g:each>
        </s2ui:tab>
